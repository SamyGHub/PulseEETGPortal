...
@Component({
  selector: 'app-portsofdischarge',
  templateUrl: './portsofdischarge.component.html',
  styleUrls: ['./portsofdischarge.component.css'],
  providers: [DatePipe]
})
export class PortsofdischargeComponent 
{
  ...
  getports()
  {
    ...
    let param: any = {};
    param.op = "query";
    param.query = "select * from vw_Wrx_Ports;";
    let headers = new HttpHeaders();
    let url = this.generalservice.appconfigs.URLs.apiUrl+"/api/query";
    this.http.post(url, param, {headers: headers}).subscribe(
    (res) => 
  ...
  deleteSelectedPort(port:any)
  {
    this.confirmationService.confirm({
      ...
      accept: () => {
        ...
        param.entity = "Wrx_DischargePorts";
        let headers = new HttpHeaders();
        this.http.post(url, param, {headers: headers}).subscribe(
        (res) => 
          {
